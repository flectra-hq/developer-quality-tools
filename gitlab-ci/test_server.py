...
def get_server_path(flectra_full, flectra_version, gitlab_home):
    """
    Calculate server path
    :param flectra_full: Flectra repository path
    :param flectra_version: Flectra version
    :param gitlab_home: Travis home directory
    :return: Server path
    """
    flectra_version = flectra_version.replace('/', '-')
    flectra_org, flectra_repo = flectra_full.split('/')
    server_dirname = "%s-%s" % (flectra_repo, flectra_version)
    server_path = os.path.join(gitlab_home, server_dirname)
    return server_path
...
def get_server_script(server_path):
    if os.path.isfile(os.path.join(server_path, 'flectra-bin')):
        return 'flectra-bin'
    return 'flectra-bin'
...
def setup_server(db, flectra_unittest, tested_addons, server_path, script_name,
    ...
    :param flectra_unittest: Boolean for unit test (gitlab parameter)
    :param tested_addons: List of modules that need to be installed
    :param server_path: Server path
    :param gitlab_build_dir: path to the modules to be tested
    :param addons_path: Addons path
    :param install_options: Install options (gitlab parameter)
    :param server_options: (list) Add these flags to the Flectra server init
    """
    ...
    try:
        ...
        print("Using previous flectra_template database.")
    else:
...
def create_server_conf(data, version):
    '''Create (or edit) default configuration file of flectra
    :params data: Dict with all info to save in file'''
    fname_conf = os.path.expanduser('~/.flectra_serverrc')
...
def main(argv=None):
    ...
    gitlab_home = os.environ.get("HOME", "~/")
    gitlab_dependencies_dir = os.path.join(gitlab_home, 'dependencies')
    gitlab_build_dir = os.environ.get("CI_PROJECT_DIR", "../..")
    flectra_unittest = str2bool(os.environ.get("UNIT_TEST"))
    flectra_exclude = os.environ.get("EXCLUDE")
    flectra_include = os.environ.get("INCLUDE")
    ...
    flectra_version = os.environ.get("VERSION")
    flectra_branch = os.environ.get("FLECTRA_BRANCH")
    ...
    dbtemplate = os.environ.get('MQT_TEMPLATE_DB', 'flectra_template')
    database = os.environ.get('MQT_TEST_DB', 'flectra_test')
    if not flectra_version:
...
#         else:
    ...
    tested_addons_list = get_addons_to_check(gitlab_build_dir,
                                             flectra_include,
                                             flectra_exclude)
    tested_addons = ','.join(tested_addons_list)

    print("Working in %s" % gitlab_build_dir)
    print("Using repo %s and addons path %s" % (flectra_full, addons_path))
    ...
    preinstall_modules = list(set(preinstall_modules or []) - set(get_modules(
        os.environ.get('CI_PROJECT_DIR')) or [])) or ['base']
    print("Modules to preinstall: %s" % preinstall_modules)
    setup_server(dbtemplate, flectra_unittest, tested_addons, server_path,
    ...
    cmd_flectra_test += options + ["--init", None]
    ...
    for to_test in to_test_list:
        ...
        for command, check_loaded in commands:
            ...
            errors = has_test_errors(
                "stdout.log", database, flectra_version, check_loaded)
            if returncode != 0:
        ...
        if not instance_alive and flectra_unittest:
