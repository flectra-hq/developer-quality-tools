...
def get_extra_params(flectra_version):
    """Get extra pylint params by flectra version
    Transform a seudo-pylint-conf to params,
    it to overwrite base-pylint-conf values.
    Use a seudo-inherit of configuration file.
    To avoid have a 2 config files (stable and pr-conf) by each flectra-version
    Example:
    ...
    :param version: String with name of version of flectra
    :return: List of extra pylint params
    """
    is_version_number = re.match(r'\d+\.\d+', flectra_version)
    beta_msgs = get_beta_msgs()
    extra_params_cmd = [
        '--sys-paths', os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            'pylint_deprecated_modules'),
        '--extra-params', '--load-plugins=pylint_flectra']

    extra_params = list(extra_params_cmd)
    if is_version_number:
        extra_params.extend([
            '--extra-params', '--valid_flectra_versions=%s' % flectra_version])

    flectra_version = flectra_version.replace('.', '')
    version_cfg = os.path.join(
        os.path.dirname(os.path.realpath(__file__)),
        'cfg/gitlab_run_pylint_exclude_%s.cfg' % (flectra_version))
    params = []
...
def get_beta_msgs():
    ...
    beta_cfg = os.path.join(
        os.path.dirname(os.path.realpath(__file__)),
        'cfg/gitlab_run_pylint_beta.cfg')
...
def get_branch_base():
    branch_base = os.environ.get('VERSION')
    if branch_base != 'HEAD':
...
@click.command()
@click.option('paths', '--path', envvar='CI_PROJECT_DIR',
...
def main(paths, config_file, msgs_no_count=None,
    ...
    to check fails of flectra modules.
